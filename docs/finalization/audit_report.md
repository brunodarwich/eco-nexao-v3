...
| Perfil sem impacto pessoal | Concluído | PARTIAL | ADR 0009, `/me`, tela e teste focal | `/me/impact`, fórmulas CO₂/score e selo pessoal removidos localmente; migration ainda não promovida | drift de deploy enquanto API/schema não forem promovidos juntos | Homologar perfil e promover migration por ambiente | ECO-1904 |
...
| Viagens e visitas | Concluído | PARTIAL | GET/POST de trips, CTA de início e histórico preservados | Sem PATCH concluir/cancelar e registro de visitas completo | ciclo factual incompleto | Completar ciclo de domínio sem atribuir impacto ambiental | ECO-1904 |
| Selos editoriais | Concluído | PARTIAL | `green_badge_status` de atores/rotas preservado; selo pessoal removido pelo ADR 0009 | Critérios/evidências editoriais ainda incompletos | greenwashing territorial | Definir política editorial auditável | ECO-1303 |
...
| Alertas | Concluído | PARTIAL | API administrativa cria, atualiza, lista e resolve alertas com janela timezone-aware, lock pessimista e audit `before`/`after`; suíte backend verde | `resolved_by` ainda não é persistido e falta smoke concorrente real | médio | Fechar persistência/concorrência na ECO-1604 | ECO-1604 |
| Rascunho/revisão/publicação/arquivo | Fora do escopo antigo | PARTIAL | State machine, Publish Guard fail-closed, segregação e versão sempre aplicada pela API; estado é travado antes do guard/transição | Recursos lidos pelo guard não compartilham lock com todos os CRUDs; visibilidade pública precisa ser uniformizada por tipo | alto | Concluir protocolo atômico e projeção pública | ECO-1604 |
...
| Reconciliação editorial | Concluída | PARTIAL | API exige decisão humana/motivo, lock pessimista, arquivo sem delete e snapshot compensável; migration e smoke básico em test | Smoke ainda não prova merge/compensação completa e reativação administrativa de vínculo arquivado precisa contrato explícito | médio | Fechar cenários reais e reativação | ECO-1604 |
...
| Painel administrativo | Fora do escopo antigo | PARTIAL | Shell administrativo acessível por `/admin`, ligado à sessão e ao `/admin/context`; teste focal cobre 403 e logout | Login editorial/MFA, separação de erro recuperável, módulos reais, revisão/auditoria e homologação visual ainda ausentes | alto risco humano | Consolidar ECO-1801 antes dos módulos ECO-1802–1804 | ECO-1801–ECO-1804 |
...
| EXIF/resize/otimização | Concluído | PARTIAL | Schema, processador e lifecycle local geram WebP `thumb`/`card`/`hero`, persistem estados e compensam falhas; testes focais 8/8 e suíte funcional 297/297 | Cobertura 84,12% abaixo do gate; API/cleanup sem teste integrado remoto, fila durável ou CAS | PII, custo, concorrência | Fechar cobertura/concorrência e executar smoke test isolado | ECO-1702 |
| Alt text/crédito/licença | ADR 0008 Aceito | PARTIAL | Constraints e Publish Guard exigem alt, crédito e licença estruturada para `ready`; Google proxy não armazena binário e expira em até 30 dias | Serviço/API/UI de processamento ainda pendentes | direitos e acessibilidade | Concluir ECO-1702 e integrar UI | ECO-1305, ECO-1702 |
...
| Endpoints administrativos | Marcados implicitamente | PARTIAL | ECO-1601 adicionou fronteira `/admin/context`, contrato OpenAPI/TS e autorização por membership/capability/escopo com matriz HTTP | CRUD, jobs, export e mutations idempotentes pertencem a ECO-1602–1605 | operação ainda incompleta | Implementar recursos administrativos sobre a fronteira revisada | ECO-1602–ECO-1605 |
| Autorização editorial | ADR 0006 aceito | PARTIAL | Serviço deny-by-default; migration `20260813175721` aplicada em test; migrations 15/15 e advisors limpos; versão efetiva e lock de estado reduzem TOCTOU | Falta lock/protocolo comum entre Publish Guard e mutations de rota/ator/origem/geometria/mídia; revisão cruzada reprovou conclusão | alto | Concluir ECO-1604 antes do painel de revisão | ECO-1604 |
